...
export default function EditServicePage({ params }: PageProps) {
  ...
  const fetchServiceData = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        // Set form values
        setValue('cardInfo.title', service.cardInfo?.title || '');
        setValue('cardInfo.description', service.cardInfo?.description || '');
        setValue('serviceBlog.title', service.serviceBlog?.title || '');
        setValue('serviceBlog.description', service.serviceBlog?.description || '');
        setValue('youtubeLinks', service.serviceBlog?.youtubeLinks || []);
        setValue('isActive', service.isActive);
        
        // Set image previews
        if (service.cardInfo?.image) {
          setImagePreview((service.cardInfo.image as any)?.url || (service.cardInfo.image as any));
        }
        if (service.serviceBlog?.heroImage) {
          setHeroImagePreview((service.serviceBlog.heroImage as any)?.url || (service.serviceBlog.heroImage as any));
        }
      }
    } catch (error: any) {
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>, type: 'card' | 'hero') => {
    ...
    if (file) {
      ...
      if (type === 'card') {
        ...
        setValue('serviceBlog.heroImage', file);
        const reader = new FileReader();
  ...
  const removeImage = (type: 'card' | 'hero') => {
    if (type === 'card') {
      ...
      setValue('serviceBlog.heroImage', null);
  ...
  const onSubmit = async (data: ServiceData) => {
    try {
      ...
      if (!data.serviceBlog.title?.trim()) {
        toast.error('Service title is required');
        return;
      }
      if (!data.serviceBlog.description?.trim()) {
      ...
      const payload: any = {
        ...
        serviceBlog: {
          title: data.serviceBlog.title,
          description: data.serviceBlog.description,
          youtubeLinks: data.youtubeLinks || [],
        },
        isActive: data.isActive,
      };

      const response = await api.put(`/api/services/${resolvedParams.id}`, payload);
  ...
  if (loading && !serviceData) {
    return (
      <DashboardLayout title="Edit Service" subtitle="Update service content">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-gray-600 mt-1">Update your service content</p>
            </div>
          </div>
          <div className="flex items-center space-x-3">
            <button
              onClick={() => router.push(`/dashboard/services/${resolvedParams.id}`)}
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div className="space-y-6">
              <div>
                ...
                <input
                  {...register('serviceBlog.title')}
                  type="text"
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent"
                  placeholder="Enter service title"
                />
                {(errors as any).serviceBlog?.title && (
                  <p className="mt-1 text-sm text-red-600">{(errors as any).serviceBlog.title.message}</p>
                )}
              </div>
              ...
              <div>
                ...
                <textarea
                  {...register('serviceBlog.description')}
                  rows={6}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent"
                  placeholder="Enter service description"
                />
                {(errors as any).serviceBlog?.description && (
                  <p className="mt-1 text-sm text-red-600">{(errors as any).serviceBlog.description.message}</p>
                )}
              </div>
